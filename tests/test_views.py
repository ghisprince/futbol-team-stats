...
class TestViews:
    ...
    def test_team_post(self, testapp):
        ...
        team1 = resp[0]
        assert sorted(list(team1)) == sorted(['_links', 'id', 'name'])
        assert team1['id'] == 1
        assert team1['name'] == "Team T"
        assert sorted(list(team1['_links'])) == ['collection', 'matches',
                                                 'players', 'self']
    ...
    def test_playermatch_post_patch(self, testapp):
        ...
        code, pmatch = get_result(testapp.patch('/api/v1/playermatches/2',
                                               data=json.dumps(player_match_patch ),
                                               content_type='application/json'))
    ...
    def test_shot(self, testapp):
        ...
        shot_data = {"player_match": pm, "x": 1, "y": 10, "on_goal": True, "pk": False}
        ...
        # POST another
        shot_data = {"player_match": pm, "x": 20, "y": 2, "on_goal": False, "pk": True}
    ...
    def test_goal_assist(self, testapp):
        ...
        assert len(goals) == 1
        assert goals[0]['assist']['_links']['self'] == assists[0]['_links']['self']
    ...
    def test_shotagainst(self, testapp):
        ...
        code, shotagainst_1 = get_result(testapp.post('/api/v1/shots/',
                                            data=json.dumps(shotagainst_data_1),
                                            content_type='application/json'))
        assert code == 201
        ...
        shotagainst_data_2 = {"player_match": pm, "x": 3, "y": 4,
                              "on_goal": False, "pk": True}

        code, shotagainst_2 = get_result(testapp.post('/api/v1/shots/',
                                            data=json.dumps(shotagainst_data_2),
                                            content_type='application/json'))
        assert code == 201
